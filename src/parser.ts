...
export function resolveDtsExports(pkgDir: string): Set<string> {
...
function resolveSourceEntry(pkgDir: string): string | null {
  try {
    const pkgJson = JSON.parse(fs.readFileSync(path.join(pkgDir, 'package.json'), 'utf8')) as {
      source?: string; module?: string; main?: string;
    };
  ...
  for (const rel of SOURCE_ENTRY_CANDIDATES) {
    if (isFile(path.join(pkgDir, rel))) return path.join(pkgDir, rel);
  }
  return null;
}
 ...
 */
export function resolveSourceExports(pkgDir: string): Set<string> {
  const names = new Set<string>();
 ...
 */
export function resolveDsSurface(pkg: string, cwd: string, libraryPath?: string): Set<string> {
  ...
  if (libraryPath) {
    ...
    if (names.size === 0) names = resolveSourceExports(dir);
    if (names.size > 0) return names;
  }
